...
from calcpy import unique, concat, union, intersect, exclude, symmetricdiff, eq, ne
...
def test_all():
    ...
    assert eq(symmetricdiff(*lists), xor_list)
    ...
    assert eq(symmetricdiff(*tuples), xor_tuple)
    ...
    assert eq(symmetricdiff(*sets), xor_set)
    ...
    assert eq(symmetricdiff(*arrs), xor_arr, matcher=np.array_equal)
    ...
    assert eq(symmetricdiff(*ss), xor_s, matcher=PandasFrameMatcher())
    ...
    assert eq(symmetricdiff(*dfs), xor_df, matcher=PandasFrameMatcher())
